...
import { EVENTS_MAP_BIG, EVENTS_MAP_SMALL, QUESTIONS_MAP_SMALL, QUESTIONS_MAP_BIG, HELPS } from 'src/environments/environment';
import { getRemovedPlayersNames } from 'src/app/helpers/get-removed-players-names';
...
export class GameComponent implements OnInit, OnDestroy {
  ...
  checkForGameActivity(): void {
    this.lastUpdateSubscription = this.timerService.getLastUpdateGame(this.board.game.players[this.user.uid].turnIndex)
      .subscribe(async v => {
        if (v === -999) {
  ...
  handleGameEvent(game: Game): void {
    ...
    this.isMyGameEvent = game.gameEvent.playerUid === this.user.uid;
    this.activeGameEvent = game.gameEvent;
    this.moveInterval = game.gameEvent.id === 'gameEvent_CUSTOM' ? 100 : 500;
