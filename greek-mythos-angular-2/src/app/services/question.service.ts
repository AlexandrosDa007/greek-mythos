...
export class QuestionService {
    ...
    getRandomQuestion(playerUid: string): Promise<GameQuestion> {
        ...
        return firstValueFrom(this.db.object<any>(`questions/${id}`).valueChanges().pipe(
            map(question => {
                return {
                    question,
                    playerUid,
    ...
    getAnswerForQuestion(questionId: string): Promise<string> {
        return firstValueFrom(this.db.object<string>(`questionAnswers/${questionId}`).valueChanges()).then(v => v ?? '');
    }
}
